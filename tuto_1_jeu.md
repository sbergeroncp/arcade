...
Ajoute le bloc ``||scene:démarrer effet sur l'écran||`` sous le bloc ``||scene:définir l'image d'arrière-plan||``.

Choisis un effet dans la liste.
...
Appuie sur le bouton ``||scene:+||`` du bloc ``||scene:démarrer effet sur l'écran||``.

Modifie la valeur ``||scene:500ms||`` par ``||scene:5000ms||``.
...
Teste l'écran d'accueil du jeu dans le simulateur.